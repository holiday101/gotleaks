...
# Overridable via the DB_PATH env var so this copy (living in GotLeaks/backend/app,
# not next to a "data/" folder of its own) points at the same live SQLite file the
# original Neptune checkout and the production server both use -- never a duplicate.
DB_PATH = os.environ.get(
    "DB_PATH",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "neptune.db"),
)
